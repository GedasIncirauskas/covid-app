...
class MapInfo extends Component {
	...
	onRegionClick (e, code) {
		...
		let findCountryCode = this.props.tableInfo.find(item => item.CountryCode === code)
		if(!findCountryCode) {
			return
		}

		axios.get(`/country/${findCountryCode.Slug}?from=${this.state.dateChangeHand.startDate}T00:00:00Z&to=${this.state.dateChangeHand.endDate}T00:00:00Z`)
			.then(response => {
				let dateChange = response.data.map(item => {
					item.Date = item.Date.slice(5, 10);
					return item;
				})
				this.setState({chartInfo: dateChange, title: response.data[0].Country, code: code})
			})
	}
				        ...
				        <div style={{display: 'inline-flex'}}>
				        	<form >
						      	<TextField
						        	...
						        	onChange={(event) => this.dateChangeHandler(event.target.value, 'startDate')} 
							        id="date"
							        label="Date from:"
							        type="date"
							        InputLabelProps={{
    						...
    						<form >
						      	<TextField 
					        		...
					        		onChange={(event) => this.dateChangeHandler(event.target.value, 'endDate')}
							        id="date2"
							        label="Date to:"
							        type="date"
							        InputLabelProps={{
					  	...
					  	{this.state.chartInfo.length > 0 ? <Chart2 chartInfo={this.state.chartInfo} title={this.state.title}/> : null}
			      </CardContent>
			    </Card>
		)
	}
