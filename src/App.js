...
class App extends Component {
  ...
  componentWillMount() {
    axios
      .get("/summary")
      .then((response) => {
        let sortedCountries = response.data.Countries.sort(
  ...
  fullCountryInformation(slug, countryName) {
    axios.get(`/total/country/${slug}`).then((response) => {
      this.setState({
        modalFullInfo: {
      ...
      <Grid container className="MuiPaper-root">
        <Grid item xs={12} sm={3}>
          ...
          <CountryTable
            ...
            fullInfo={this.fullCountryInformation}
          />
          <Modal
            modalInfo={this.state.modalFullInfo}
